...
#ifndef RIPPER
static ID register_symid(ID, const char *, long, int);
...
static size_t
ivar2_memsize(const void *ptr)
{
    size_t size = 0;

    if (ptr) {
	const struct ivar_symbols *isym = ptr;
	if (isym->ivar2_id) {
	    size += st_memsize(isym->ivar2_id);
	    size += isym->ivar2_id->num_entries * sizeof(struct ivar2_key);
	}
	if (isym->id_ivar2) size += st_memsize(isym->id_ivar2);
    }
    return size;
}

static const rb_data_type_t ivar2_data_type = {
    "selector-namespace",
    ivar2_mark, ivar2_free, ivar2_memsize,
};
...
ID
rb_ns_intern(VALUE iw, VALUE klass, ID id)
{
    struct ivar_symbols *isym = rb_check_typeddata(iw, &ivar2_data_type);
    struct ivar2_key key, *newkey = 0;
    st_data_t data;
    key.id = id;
    key.klass = klass;
    if (st_lookup(isym->ivar2_id, (st_data_t)&key, &data)) {
	return (ID)data;
    }
    id = internal_id();
    newkey = ALLOC(struct ivar2_key);
    *newkey = key;
    st_add_direct(isym->ivar2_id, (st_data_t)&key, (st_data_t)id);
    st_add_direct(isym->id_ivar2, (st_data_t)id, (st_data_t)&key);
    return id;
}
#endif
...
InitVM_sym(void)
{
    ...
    VALUE iw = TypedData_Wrap_Struct(0, &ivar2_data_type, isym);
...
static VALUE
sym_str_new(const char *name, long len, int encidx)
{
	...
	ptr = rb_objspace_xmalloc(global_symbols.objspace, len+1);
	RSTRING(str)->basic.flags |= RSTRING_NOEMBED;
...
static ID
register_symid(ID id, const char *name, long len, int encidx)
{
    VALUE str = sym_str_new(name, len, encidx);
...
rb_intern3(const char *name, long len, rb_encoding *enc)
{
    ...
    if (&rb_cString && rb_cString && !rb_enc_asciicompat(enc)) {
	id = ID_JUNK;
	goto new_id;
    }
    last = len-1;
  ...
  id_register:
    return register_symid(id, name, len, rb_enc_to_index(enc));
}
...
rb_id2str(ID id)
{
	...
	if (id < INT_MAX && rb_ispunct((int)id)) {
		...
		name[0] = (char)id;
		name[1] = 0;
		str = sym_str_new(name, 1, rb_usascii_encindex());
	...
	for (i = 0; i < op_tbl_count; i++) {
		...
		if (!str) {
		    ...
		    str = sym_str_new(name, strlen(name), rb_usascii_encindex());
		    global_symbols.op_sym[i] = str;
		}
		return str;
	    }
