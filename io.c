...
static int
rb_sysopenat(VALUE fname, const struct openat_args *base, int oflags, mode_t perm)
{
    int fd;
	...
	fname = fullpath = rb_file_absolute_path(fname, fullpath);
		     ...
		     int oflags, int fmode, convconfig_t *convconfig, mode_t perm)
{
    ...
    fptr->fd = rb_sysopenat(fptr->pathv, base, oflags, perm);
    io_check_tty(fptr);
