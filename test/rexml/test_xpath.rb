...
class XPathTester < Test::Unit::TestCase
    ...
    assert_equal("c", c.name)
		...
		doc = REXML::Document.new(File.new("test/rexml/data/project.xml"))
...
<a>
  ...
  def test_ticket_56
    ...
    finaldoc = REXML::Document.new(File.read('test/rexml/data/google.2.xml'))
...
EOF
  ...
  def test_ticket_43
    ...
    sum = Document.new(File.new("test/rexml/data/yahoo.xml")).elements.to_a("//item").size
    assert_equal( 10, sum )

    text = Document.new(File.new("test/rexml/data/yahoo.xml")).elements.to_a(%Q{//title[contains(text(), "'")]}).collect{|e| e.text}.join
