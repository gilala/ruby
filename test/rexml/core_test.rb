...
class Tester < Test::Unit::TestCase
  ...
  def test_doctype
    ...
    docin = File.new "test/rexml/data/doctype_test.xml"
  ...
  def test_document
    ...
    source = File.new( "test/rexml/data/dash.xml" )
    doc = Document.new source
  ...
  def test_source
    ...
    # Testing IO source
    doc = Document.new File.new("test/rexml/data/project.xml")
  ...
  def test_big_documentation
    f = File.new("test/rexml/data/documentation.xml")
    d = Document.new f
  ...
  def test_tutorial
    doc = Document.new File.new("test/rexml/data/tutorial.xml")
    out = ""
    doc.write out
  end
  ...
  def test_stream
    c = Listener.new
    Document.parse_stream( File.new("test/rexml/data/documentation.xml"), c )
  ...
  def test_line
    doc = Document.new File.new( "test/rexml/data/bad.xml" )
...
<!ELEMENT bar (#PCDATA)>
  ...
  def test_attlist_write
    file=File.new("test/rexml/data/foo.xml" ) 
...
<files>
  ...
  def test_repeated_writes
    require 'iconv'
    a = IO.read( "test/rexml/data/iso8859-1.xml" )
  ...
  def test_ticket_63
    d = REXML::Document.new( File.new("test/rexml/data/t63-1.xml") )
  end

  def test_ticket_75
    d = REXML::Document.new( File.new("test/rexml/data/t75.xml") )
